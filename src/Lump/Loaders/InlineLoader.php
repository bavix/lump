...
/**
 * A lump Template loader for inline templates.
 *
 * With the InlineLoader, templates can be defined at the end of any PHP source
 * file:
 *
 *     $loader  = new Loader_InlineLoader(__FILE__, __COMPILER_HALT_OFFSET__);
 ...
 *
 *     $app->register(new lumpServiceProvider, array(
 *         'lump.loader' => new Loader_InlineLoader(__FILE__, __COMPILER_HALT_OFFSET__)
 *     ));
 *
 *     $app->get('/{name}', function ($name) use ($app) {
 *         return $app['lump']->render('hello', compact('name'));
 *     })
 *     ->value('name', 'world');
 *
 *     // ...
 ...
 *     Hello, {{ name }}!
 */
class InlineLoader implements LoaderInterface
{
    protected $fileName;
    protected $offset;
    protected $templates;
    ...
    public function __construct($fileName, $offset)
    {
        if (!is_file($fileName))
        {
            throw new Exceptions\InvalidArgumentException('InlineLoader expects a valid filename.');
        }

        if (!is_int($offset) || $offset < 0)
        {
            throw new Exceptions\InvalidArgumentException('InlineLoader expects a valid file offset.');
        }
